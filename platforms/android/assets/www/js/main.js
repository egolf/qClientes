...
function MapCtrl(onFail) {
    ...
    this.locateAddress = function() {
        //$(this.headerID).text("Carregando Mapa...");
        //$.mobile.loading('show');
        $.mobile.loading('show', {theme:"e", text:"Carregando Mapa...", textonly:true, textVisible: true});
        google.load("maps", "3.8", {"callback": function() {
                $.mobile.loading('hide');
                //$(this.headerID).text("Cliente");
                showAddressOnMap(self.address, self.mapContainter, self.headerID);
            }, other_params: "sensor=true&language=pt-br&libraries=places"});
    };
